...
- **`FinanceSmith.Banking.BudgetTarget`** mirrors MetaCategory household policies: reads/updates/destroys require `household_id == ^actor(:household_id)`; `:create` requires a non-nil actor `household_id` and stamps `household_id` from the actor. No `authorize?: false` on user-facing BudgetTarget read/write paths.
