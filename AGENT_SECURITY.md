...
- **NEVER** load `plaid_item.access_token` for bulk UI or broad `Ash.read!` lists. Load and use it only in the three code paths that perform Plaid API calls:
  1. **Sync pipeline** — `FinanceSmith.DataLake.SyncWorker` and its helper `FinanceSmith.Banking.BalanceRefresh`. `BalanceRefresh.run/2` receives an already-loaded struct; it never performs its own `access_token` read.
...
- **`FinanceSmith.Banking.MetaCategory`** and **`FinanceSmith.Banking.CategoryMapping`** declare `Ash.Policy.Authorizer`. Reads require `household_id == ^actor(:household_id)`. MetaCategory user-facing `:create` requires a non-nil actor `household_id`; `household_id` is stamped from the actor (`SetHouseholdFromActor`). MetaCategory `:update`/`:destroy` require the same household match. CategoryMapping writes are system-only (`forbid_if always()`); Oban workers and seeds call `authorize?: false`.
- **`FinanceSmith.Banking.BudgetTarget`** mirrors MetaCategory household policies for reads/updates/destroys (`household_id == ^actor(:household_id)`). `:create` stamps `household_id` from the actor and additionally requires `meta_category.household_id == ^actor(:household_id)`, enforced post-insert inside the action's transaction so a target can never point at another household's category. No `authorize?: false` on user-facing BudgetTarget read/write paths.
