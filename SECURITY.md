...
- **Use:** The token is consumed inside the **Oban-backed sync pipeline** ([`SyncWorker`](lib/finance_smith/data_lake/sync_worker.ex)) when calling Plaid, and in the **actor-authorized balance refresh** path ([`:fetch_realtime_balances`](lib/finance_smith/banking/plaid_item/changes/fetch_realtime_balances.ex)) — a tightly scoped `before_action` load that never returns the token to the UI. It must not be loaded for bulk UI listing of items.
- **Hybrid balance model `[ACTIVE]`:** Transaction sync responses include free cached balances. [`SyncWorker`](lib/finance_smith/data_lake/sync_worker.ex) claims a 24-hour paid `/accounts/balance/get` window, issues the paid fetch first, and applies sync-payload cached balances only as a same-run fallback when that paid call fails and the claim is still held. Users may trigger an intentional refresh from [`AccountLive`](lib/finance_smith_web/live/account_live.ex) with a cost advisory when the paid-refresh window was recently claimed. Implementation rules and permitted load sites: **[AGENT_SECURITY.md](AGENT_SECURITY.md)** (rule 6).
...
[`FinanceSmith.Banking.PlaidItem`](lib/finance_smith/banking/plaid_item.ex), [`Account`](lib/finance_smith/banking/account.ex), and [`Transaction`](lib/finance_smith/banking/transaction.ex) use **`Ash.Policy.Authorizer`**, consistent with **`User`** (subsection above). Reads are authorized for the actor via a two-branch expression:
