...
- **Hybrid balance model `[ACTIVE]`:** Transaction sync responses include free cached balances, applied at end-of-sync by [`SyncWorker`](lib/finance_smith/data_lake/sync_worker.ex). Paid `/accounts/balance/get` calls are gated to a 24-hour window in the background worker; users may trigger an intentional refresh from [`AccountLive`](lib/finance_smith_web/live/account_live.ex) with a cost advisory when balances are fresh. Implementation rules and permitted load sites: **[AGENT_SECURITY.md](AGENT_SECURITY.md)** (rule 6).
