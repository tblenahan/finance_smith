defmodule FinanceSmith.Identity.User do
  ...
  aggregates do
    ...
    sum :outflow_30d, [:plaid_items, :accounts, :transactions], :amount do
      ...
      join_filter [:plaid_items, :accounts], expr(is_nil(duplicate_of_id))
      default 0
    end
    ...
    sum :total_credit, [:plaid_items, :accounts], :credit_limit do
      filter expr(type == "credit" and status == :active and is_nil(duplicate_of_id))
      default 0
    end

    sum :total_available_credit, [:plaid_items, :accounts], :available_credit do
      filter expr(type == "credit" and status == :active and is_nil(duplicate_of_id))
      default 0
    end
  end
